...
def configure_web_driver():
	...
	# a hack
	if (os.name=='nt'):
		driver = webdriver.Chrome(chrome_options=chrome_options,executable_path='chromedriver.exe')#windows
	else: #(os.name=='posix'):
...
def compile_logs(log_folder,interval_update=False):
	...
	with open((os.path.join(log_folder, "aggregate log")),'r') as agg_log:
		rows = agg_log.read().strip().split(' ')

	rows = np.array(np.unique([int(row) for row in rows if row.isdigit()]))
	return rows
...
def download_data_chunk(chunk):
	...
	process, latency, download,download_entire,iotime = [],[],[],[],[]
	counter = 0
	with open(os.path.join(log_folder,log_name),"a") as log:
		for row_index, row in chunk[chunk['download']==0].iterrows():
			...
			filing_folder_name = ' '.join([str(row_index),str(cik),str(date_str),str(filing)])
			...
			try:
				...
				with open(filing_folder_path+'/'+'download.log', 'w', newline='') as download_log:
				    ...
				    for a in driver.find_elements_by_xpath('.//a'):
				    	...
				    	if(url.startswith(PREFIX)):
				    		...
				    		download_time += time.time() - start_
				...
				latency.append(stamps[2]-stamps[1])
				download.append(download_time)
				download_entire.append(stamps[4]-stamps[3])
				iotime.append(stamps[3]-stamps[2]+stamps[5]-stamps[4])
			...
			if( counter % 100==0):
				...
				print("For {} filings. Process avg: {:.3f} seconds. Latency avg: {:.3f} seconds. Documents downloading avg: {:.3f}. Documents downloading(including process and io) avg: {:.3f} Iotime avg: {:.3f}".format(str(counter),np.mean(process),np.mean(latency),
					np.mean(download),np.mean(download_entire),np.mean(iotime)))
...
if __name__ == '__main__':
	...
	database_path = "Database files/{}-QTR1 to 2018-QTR2.csv".format(str(year))
	if not os.path.exists(database_path):
		database = IndexDownloader.download_and_convert(year=year)
	else:
